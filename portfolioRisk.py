...
for p in positions:
    ...
    base  = p.contract.symbol.upper()   # the CFD's displayed "symbol"
    quote = p.contract.currency.upper() # the CFD's displayed "currency"
    qty   = float(p.position)

    # price series for the contract pair (BASE/QUOTE)
    s_bq = _pair_series(base, quote)
    if s_bq is None or s_bq.empty:
        continue
    px_pair = float(s_bq.iloc[-1])  # latest
