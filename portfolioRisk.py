...
LOOKBACK = '5 Y'
BAR = '1 day'
SUPPORTED_SEC_TYPES = {'STK', 'ETF', 'CASH', 'CFD', 'CMDTY'}
...
def get_equity_series(qc: Contract, dur=LOOKBACK, bar=BAR, whatToShow='TRADES') -> Optional[pd.Series]:
    bars = ib.reqHistoricalData(
        qc, 
        endDateTime='', 
        durationStr=dur,
        barSizeSetting=bar, 
        whatToShow=whatToShow, # This now uses the variable passed in
        useRTH=False, 
        formatDate=1
    )
    return _bars_to_series(bars)
...
def get_equity_last_close_local(qc: Contract) -> Tuple[Optional[float], str]:
    ...
    show_type = 'MIDPOINT' if qc.secType == 'CMDTY' else 'TRADES'
    ...
    try:
        bars = ib.reqHistoricalData(
            ...
            whatToShow=show_type, # Use the dynamic show_type
            useRTH=False, 
            formatDate=1
        )
        ...
        df = util.df(bars)
        return float(df.iloc[-1]['close']), f"HMDS:1day {show_type}"
...
def normalize_contract_from_position(p: Position) -> Optional[Contract]:
    ...
    if sec == 'CMDTY':
        # Spot Gold/Silver require SMART exchange and USD currency in the API
        return Commodity(sym, 'SMART', cur)
    return None
